...
const storePost = (req, res) => {
    ...
    form.parse(req, (err, fields, files) => {
        ...
        if (title.length === 0) {
            errors.push({ msg: "Title is requires" })
        }
        if (body.length === 0) {
            errors.push({ msg: "Body is requires" })
        }
        ...
        if (files.image.name.length === 0) {
            ...
            errors.push({ msg: "Only jpg and png files are allowed" });
        }
                                    ...
                                    try{
                                           ...
                                           const name = user.name;
                                            ...
                                            try{
                                                 ...
                                                 if(result){
                                                    req.flash('success',"Your account has been added sucessfully")
...
const updateForm= async (req,res)=>{
    ...
    res.render("update",{title:"UpdatePost",login:true,errors:[],post})
...
const postValidations = [
    check('title').not().isEmpty().withMessage("Title is requires"),
...
const postUpdate =async(req,res)=>{
    ...
    res.render("update",{title:"UpdatePost",login:true,errors:errors.array(),post});
    }else{
        ...
        try{    
            ...
            if(updateResult){
                req.flash("Your Post have been updated")
                res.redirect('/posts/1')
            }
        }catch(err){
            res.send(err)
